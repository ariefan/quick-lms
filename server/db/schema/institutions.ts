 ...
 */
export const institutionMembers = sqliteTable("institution_members", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  ...
  role: text("role", { enum: ["admin", "instructor"] }).default("instructor").notNull(),

  // Permissions
  canCreateCourses: integer("can_create_courses", { mode: "boolean" }).default(true).notNull(),
  canEditAllCourses: integer("can_edit_all_courses", { mode: "boolean" }).default(false).notNull(),
  ...
  invitedAt: integer("invited_at", { mode: "timestamp" }).$defaultFn(() => new Date()).notNull(),
 ...
 */
export const institutionInvitations = sqliteTable("institution_invitations", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  institutionId: text("institution_id").notNull().references(() => institutions.id, { onDelete: "cascade" }),
