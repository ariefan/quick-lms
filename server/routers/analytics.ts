...
export const analyticsRouter = createTRPCRouter({
  ...
  getInstructorCourseAnalytics: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const dateFilter = startDate && endDate
        ? and(
            gte(enrollments.createdAt, startDate),
            lte(enrollments.createdAt, endDate)
          )
        : undefined;
      ...
      if (quizIds.length > 0) {
        const quizScoreResult = await ctx.db
          ...
          .from(quizAttempts)
          .where(sql`${quizAttempts.quizId} IN ${sql.raw(`(${quizIds.map(() => '?').join(',')})`)} AND ${quizAttempts.status} = 'completed'`);
      ...
      if (assignmentIds.length > 0) {
        const assignmentScoreResult = await ctx.db
          ...
          .where(sql`${assignmentSubmissions.assignmentId} IN ${sql.raw(`(${assignmentIds.map(() => '?').join(',')})`)} AND ${assignmentSubmissions.status} = 'graded'`);
      ...
      const recentEnrollments = await ctx.db
        ...
        .where(
          and(
            eq(enrollments.courseId, courseId),
            gte(enrollments.createdAt, thirtyDaysAgo)
          )
        );
   ...
   */
  getInstructorDashboard: protectedProcedure
    .query(async ({ ctx }) => {
  ...
  getAdminDashboard: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      const totalInstitutionsResult = await ctx.db
        .select({ count: count() })
        .from(institutions);
      ...
      // Get total enrollments
      const totalEnrollmentsResult = await ctx.db
        .select({ count: count() })
        .from(enrollments);
